					...
					<div class="right my_shop" data-type="">
						<div class="shop_tab2_add">
								<div class="top">
										<div>
												<i>商品名称</i>
												<input type="text" name="name" value="<?php echo $good['title']?>" placeholder="商品名称不大于30个字" class="title">
												<div style="clear:both;"></div>
										</div>
										<div>
												<i>商品简介</i>
												<textarea placeholder="对此商品的简介" class="intro"><?php echo $good['intro']?></textarea>
												<div style="clear:both;"></div>
										</div>
										<div>
												<i>商品库存</i>
												<input type="text" name="name" value="<?php echo $good['stock']?>" class="stock">
												<div style="clear:both;"></div>
										</div>
										<div>
												<i>商品分类</i>
												<select class="cid">
													...
													<option value="<?php echo $vo['id']?>" <?php if($good['cid']==$vo['id']) echo 'selected'?>><?php echo $vo['title']?></option>
													<?php }?>
												</select>
												<div style="clear:both;"></div>
										</div>
										<div>
												<i>商品价格</i>
												<input type="text" name="name" value="<?php echo $good['price']?>" class="price">
												<div style="clear:both;"></div>
										</div>
								</div>
								<div class="center">
										...
										<ul>
											<li style="margin-left:300px"><img src="<?php echo $good['img']?>" class="shangpin_pho1" /><input type="file" name="name" value="" onchange="previewFile1()" id="scflie1"></li>
												<div style="clear:both;"></div>
										</ul>
								</div>
								<div class="bottom">
										...
										<div style="width:100%;">
												<script id="editor" type="text/plain" style="width:790px;height:490px;margin:0 auto;"><?php echo $good['details']?></script>
										</div>
								</div>
								<div class="bottom">
									<h1>商品上架： <input type="radio" name="goods_status" value="1" class="goods_status" <?php if($good['goods_status']==1) echo 'checked'?>> <b>上架</b><input type="radio" name="goods_status" value="2" class="goods_status" style="margin-left:40px;" <?php if($good['goods_status']==2) echo 'checked'?>> <b>下架</b></h1>
									<h4><a href="javascript:;" onclick="xiugai()">保存并提交</a></h4>
	...
	<input type="hidden" value="<?php echo $good['img']?>" class="my_avatar1"/>
	<input type="hidden" value="<?php echo $good['id']?>" class="goods_id"/>
	<!-- 尾部 -->
...
<script type="text/javascript">
	...
	function xiugai()
	{
		...
		if(title && intro && stock && cid && price && img && detail && goods_status){
			$.post(RestApi, { c: 'goods',a: 'edit_goods',title:title,intro:intro,stock:stock,cid:cid,price:price,img:img,detail:detail,goods_status:goods_status,goods_id:goods_id}, function(response) {
