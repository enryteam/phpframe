...
<body>
	...
	<div class="baogou_confirm">
		<h1><a href="../acenter/index.php?c=user&a=address"><img src="../attms/images/confirm_add.png" alt="" />新增收货地址</a></h1>
		<ul>
		...
		<div>
			...
			<div>
				...
				<?php if(!empty($allgoods)){foreach($allgoods as $k=>$v){?>
					<li>
						<i>
							...
							<div class="left">
								<h1><?php echo mb_substr($v["title"],0,30,'utf-8');?> </h1>
							</div>
							<h6 style="clear:both;"></h6>
						</i>
						<b><?php echo $v["price"];?></b>
						<b><?php echo $v['buynum'];?></b>
				...
				<h4><i onClick="return_shopcar()">返回购物车</i><a href="javascript:;" onClick="javascript:$('.baogou_confirm').hide();confirm_order(<?php echo $goods_id;?>)">提交订单</a></h4>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		//提交订单
		function confirm_order(goods_id){
			var demand = $('#demand').val();
			var addressid = $("input[name='address']:checked").val();
			var total_price = $('#total_price').html();
			localStorage.setItem("total_price", total_price);
			$.post(RestApi, { c: 'buy',a: 'commit_temporder',addressid:addressid,amount:total_price,goods_id:goods_id,demand:demand}, function(response) {
				console.log(response);
				var responseObj=$.parseJSON(response);
				if(responseObj.code == 500){
		...
		function payorder()
		{
			...
			$.post(RestApi, { c: 'buy',a: 'checkorder',orderids:localStorage.getItem("orderids"),tpassword:tpassword}, function(response){
				...
				{
					window.wxc.xcConfirm(responseObj.message,"../portal/index.php?c=mall&a=index");
				}else{
					window.wxc.xcConfirm(responseObj.message);
				}
