...
class bank extends BmsAction
{
	...
	public function index()
	{
		...
		$r = $Model->query("select count(1) as count from `jjs_bank` order by ctime desc");
		$count = $r[0]["count"];
		$res = $Model->query("select * from `jjs_bank` order by ctime desc limit ".($page-1)*$page_size.",".$page_size);
		$urlrule = 'index.php?c=bank&a=index';
	...
	public function add()
	{
		...
		if(isPost()){
			...
			$r = $Model->query("select * from `jjs_bank` where title = '".$title."'");
			if($r){
				bmsAlert("请勿重复添加",pfUrl("bms","bank","add")); 
			}else{
				$res = $Model->querysql("insert into `jjs_bank` (title,code,image,status,ctime) values('".$title."','".$code."','".$image."','".$status."','".time()."')");
				if($res){
					bmsAlert("添加成功",pfUrl("bms","bank","index")); 
				}else{
					bmsAlert("添加失败",pfUrl("bms","bank","add")); 
				}
			}
	...
	public function edit()
	{
		...
		$id = intval(getgpc("id"));
		if(!$id) bmsAlert("内容不存在", pfUrl(null, "bank", "index"));
		$Model=D('Bms');
		if(isPost()){
			$title = getgpc("title");
			$code = getgpc("code");
			$image = getgpc("image");
			$status = getgpc("status");
			$res = $Model->querysql("update `jjs_bank` set title = '".$title."',code = '".$code."',image = '".$image."',status = '".$status."' where id = ".$id);
			if($res){
				bmsAlert("编辑成功",pfUrl("bms","bank","index")); 
			}else{
				bmsAlert("编辑失败",pfUrl("bms","bank","edit",array('id'=>$id))); 
			}
		}
		$res = $Model->query("select * from `jjs_bank` where id = $id");
		$this->assign('bank',$res[0]);
		$this->display();
	}
	...
	public function remove()
    {
        ...
        $id = isset($_GET['id']) ? intval(getgpc("id")) : bmsAlert("内容不存在", pfUrl(null, "bank", "index"));
				$res = D('Bms')->querysql("delete from `jjs_bank` where id = ".$id);
