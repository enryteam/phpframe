...
class goods_release extends BmsAction
{
	private $pageSize = 10;
	...
	public function index()
	{
		...
		$code = getgpc("code");
		$title = getgpc("title");
		$release_status = getgpc("release_status");
		$where = "where 1";
		$urlrule = 'index.php?c=goods_release&a=index';
		if($code){
			$where .= " and code = '".$code."'";
			$urlrule = 'index.php?c=goods_release&a=index&code='.$code;
		}
		if($title){
			$where .= " and title like '%".$title."%'";
			$urlrule = 'index.php?c=goods_release&a=index&title='.$title;
		}
		if($release_status=='no'){
			$where .= " and release_status = 0";
			$urlrule = 'index.php?c=goods_release&a=index&release_status='.$release_status;
		}elseif($release_status == 1){
			$where .= " and release_status = 1";
			$urlrule = 'index.php?c=goods_release&a=index&release_status='.$release_status;
		}elseif($release_status == 2){
			$where .= " and release_status = 2";
			$urlrule = 'index.php?c=goods_release&a=index&release_status='.$release_status;
		}
		
		$r = $Model->query("select count(1) as count from `jjs_goods_release` ".$where." order by id desc");
		$count = $r[0]["count"];
		$res = $Model->query("select * from `jjs_goods_release` ".$where." order by id desc limit ".($page-1)*$page_size.",".$page_size);
	...
	public function edit()
	{
		...
		if(isPost()){
			$id = intval(getgpc("id"));
			$release_status = intval(getgpc("release_status"));
			$gid = $Model->query("select * from `jjs_goods_release` where id = ".$id);
			if($release_status>0){
				
				$res = $Model->querysql("update `jjs_goods_release` set release_status = ".$release_status." where id = ".$id);
				if($res){
					if($release_status==1){
						$Model->querysql("insert into `jjs_contract`(user_id,g_code,cate,quantity,price,lave_quantity,status,ctime) values('".$gid[0]["user_id"]."','".$gid[0]['code']."','1','".$gid[0]['num']."','".$gid[0]['price']."','".$gid[0]['num']."','0','".date("Y-m-d H:i:s")."')");
					}
					bmsAlert("操作成功",pfUrl("bms","goods_release","index")); 
				}else{
					bmsAlert("操作失败",pfUrl("bms","goods_release","edit")); 
				}
			}
		...
		$id = intval(getgpc("id"));
		if($id>0){
			$detail = $Model->query("select * from `jjs_goods_release` where id = ".$id);
