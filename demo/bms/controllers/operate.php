...
class operate extends BmsAction
{
	private $pageSize = 10;
	...
	public function index()
	{
		...
		$where = "where type = 2";
		$urlrule = 'index.php?c=operate&a=index';
		if($phone){
			$where .= " and phone = '".$phone."'";
			$urlrule = 'index.php?c=operate&a=index&phone='.$phone;
		}
		if($realname){
			$where .= " and realname = '".$realname."'";
			$urlrule = 'index.php?c=operate&a=index&realname='.$realname;
		}
		if($is_check == 0 && $is_check != ''){
			$where .= " and is_check = 0";
			$urlrule = 'index.php?c=operate&a=index&is_check='.$is_check;
		}elseif($is_check == 1){
	...
	public function edit()
	{
		...
		if(isPost()){
			...
			if($res){
				...
				bmsAlert("操作成功",pfUrl("bms","operate","index")); 
			}else{
				bmsAlert("操作失败",pfUrl("bms","operate","edit")); 
			}
		}
		$id = intval(getgpc("id"));
		if($id>0){
		...
		if(isPost())
		{
			...
			foreach($sqlArr as $key => $strs)
			{
				...
				{ 
					...
					$sql = "insert into `jjs_user`(id,referral_code,type,mechanism,realname,phone,password,idcard,address,email,bank_card,bank,accountname,business_licence,organization_code,tax_registration,businessname,businessduties,businessphone,businessidcard,ctime) values('".rand(100000000,999999999)."','".$key1."','".$strs[0]."','".$strs[1]."','".$strs[2]."','".$strs[3]."','".md5(trim($strs[17]))."','".$strs[4]."','".$strs[5]."','".$strs[6]."','".$strs[7]."','".$strs[8]."','".$strs[9]."','".$strs[10]."','".$strs[11]."','".$strs[12]."','".$strs[13]."','".$strs[14]."','".$strs[15]."','".$strs[16]."','".time()."')";
			...
			if($tag>0){
				bmsAlert("有".$tag."条记录导入失败",pcUrl("operate","index"));
			}else{
				bmsAlert("所有记录导入成功",pcUrl("operate","index"));
			}
		}
		$this->display();
	}
	...
	public function remove()
    {
        ...
        $id = isset($_GET['id']) ? intval(getgpc("id")) : bmsAlert("内容不存在", pfUrl(null, "operate", "index"));
		$res = D('Bms')->querysql("delete from `jjs_operate` where id = ".$id);

        if ($res) {

            bmsAlert("删除成功", pfUrl(null, "operate", "index"));

        } else {

            bmsAlert("删除失败", pfUrl(null, "operate", "index"));
	...
	public function development()
	{
		...
		$is_check = getgpc("is_check");
		$where = "where tuser_code in(select referral_code from `jjs_user` where type = 2)";
		$urlrule = 'index.php?c=operate&a=development';
		if($realname){
			$where .= " and realname like '%".$realname."%'";
			$urlrule = 'index.php?c=operate&a=development&realname='.$realname;
		}
		if($is_check == 0 && $is_check != ''){
			$where .= " and is_check = 0";
			$urlrule = 'index.php?c=operate&a=development&is_check='.$is_check;
		}elseif($is_check == 1){
			$where .= " and is_check = 1";
			$urlrule = 'index.php?c=operate&a=development&is_check='.$is_check;
		}elseif($is_check == 2){
	...
	public function detail()
	{
		...
		$realname = trim(getgpc("realname"));
		$is_check = getgpc("is_check");
		$urlrule = 'index.php?c=operate&a=detail';
		if($referral_code){$where = "where tuser_code = '".$referral_code."'";$this->assign("referral_code",$referral_code);}else{ $where  = " where 1";}
		if($realname){
			$where .= " and realname like '%".$realname."%'";
			$urlrule = 'index.php?c=operate&a=detail&referral_code='.$referral_code.'&realname='.$realname;
		}
		if($is_check == 0 && $is_check != ''){
			$where .= " and is_check = 0";
			$urlrule = 'index.php?c=operate&a=detail&referral_code='.$referral_code.'&is_check='.$is_check;
		}elseif($is_check == 1){
			$where .= " and is_check = 1";
			$urlrule = 'index.php?c=operate&a=detail&referral_code='.$referral_code.'&is_check='.$is_check;
		}elseif($is_check == 2){
		...
		$pages = $this->pages($count, $page, $page_size,$urlrule);
		if($count<=0){
			bmsAlert("该客户还没有发展客户", pfUrl(null, "operate", "development"));
		}
		$this->assign("res",$res);
		$this->assign("pages",$pages);
		$this->assign("count",$count);
	...
	public function deal()
	{
		...
		$where = "where C.user_id = U.id and U.type = 2 and C.status>0";
		if($referral_code){
