...
class agent extends BmsAction
{
	...
	public function index()
	{
		...
		$where = "where type = 4";
		$urlrule = 'index.php?c=agent&a=index';
		if($phone){
			$where .= " and phone = '".$phone."'";
			$urlrule = 'index.php?c=agent&a=index&phone='.$phone;
		}
		if($realname){
			$where .= " and realname = '".$realname."'";
			$urlrule = 'index.php?c=agent&a=index&realname='.$realname;
		}
		if($is_check == 0 && $is_check != ''){
			$where .= " and is_check = 0";
			$urlrule = 'index.php?c=agent&a=index&is_check='.$is_check;
		}elseif($is_check == 1){
	...
	public function edit()
	{
		...
		if(isPost()){
			...
			if($res){
				...
				bmsAlert("操作成功",pfUrl("bms","agent","index")); 
			}else{
				bmsAlert("操作失败",pfUrl("bms","agent","edit")); 
			}
		}
		$id = intval(getgpc("id"));
		if($id>0){
	...
	public function batchimport() {
		...
		if(isPost())
		{
			...
			if($tag>0){
				bmsAlert("有".$tag."条记录导入失败",pcUrl("agent","index"));
			}else{
				bmsAlert("所有记录导入成功",pcUrl("agent","index"));
			}
		}
		$this->display();
	}
	...
	public function remove()
    {
        ...
        $id = isset($_GET['id']) ? intval(getgpc("id")) : bmsAlert("内容不存在", pfUrl(null, "user", "index"));
		$res = D('Bms')->querysql("delete from `jjs_user` where id = ".$id);

        if ($res) {

            bmsAlert("删除成功", pfUrl(null, "agent", "index"));

        } else {

            bmsAlert("删除失败", pfUrl(null, "agent", "index"));
	...
	public function development()
	{
		...
		$is_check = getgpc("is_check");
		$where = "where tuser_code in(select referral_code from `jjs_user` where type = 4)";
		if($realname){
	...
	public function deal()
	{
		...
		$where = "where C.user_id = U.id and U.type = 4 and C.status>0";
		if($referral_code){
