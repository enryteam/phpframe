...
class article_cate extends BmsAction
{
	private $pageSize = 10;
	...
	public function index()
	{
		...
		$r = $Model->query("select count(1) as count from `jjs_article_cate` order by ctime desc");
		$count = $r[0]["count"];
		$res = $Model->query("select * from `jjs_article_cate` order by ctime desc limit ".($page-1)*$page_size.",".$page_size);
		$urlrule = 'index.php?c=article_cate&a=index';
		$pages = $this->pages($count, $page, $page_size,$urlrule);

		$this->assign('article_cate',$res);
	...
	public function add()
	{
		...
		if(isPost()){
			$title = getgpc("title");
			$image = getgpc("image");
			$res = $Model->querysql("insert into `jjs_article_cate` (title,image,ctime) values('".$title."','".$image."','".time()."')");
			if($res){
				bmsAlert("添加成功",pfUrl("bms","article_cate","index")); 
			}else{
				bmsAlert("添加失败",pfUrl("bms","article_cate","add")); 
			}
		}
		$this->display();
	}
	...
	public function edit()
	{
		...
		if(isPost()){
			...
			$title = getgpc("title");
			$image = getgpc("image");
			$res = $Model->querysql("update `jjs_article_cate` set title = '".$title."',image = '".$image."' where id = ".$id);
			if($res){
				bmsAlert("编辑成功",pfUrl("bms","article_cate","index")); 
			}else{
				bmsAlert("编辑失败",pfUrl("bms","article_cate","edit")); 
			}
		}
		$id = intval(getgpc("id"));
		if($id>0){
			$detail = $Model->query("select * from `jjs_article_cate` where id = ".$id);
	...
	public function remove()
    {
        ...
        $id = isset($_GET['id']) ? intval(getgpc("id")) : bmsAlert("内容不存在", pfUrl(null, "article_cate", "index"));
		$res = D('Bms')->querysql("delete from `jjs_article_cate` where id = ".$id);

        if ($res) {

            bmsAlert("删除成功", pfUrl(null, "article_cate", "index"));

        } else {

            bmsAlert("删除失败", pfUrl(null, "article_cate", "index"));
