...
class deal extends BmsAction
{
	...
	public function parameter()
	{
		...
		if(isPost()){
			$id = intval(getgpc("id"));
			$fee = getgpc("fee");
			$operate_rate = getgpc("operate_rate");
			$agent_rate = getgpc("agent_rate");
			if(empty($fee) && empty($operate_rate) && empty($agent_rate)){
				bmsAlert("请至少选择一项进行修改",pfUrl("bms","deal","parameter")); 
			}
			$condition = "update `jjs_deal_setting` set";
			if($fee){
				$condition .= " fee = '".$fee."',";
			}
			if($operate_rate){
				$condition .= " operate_rate = '".$operate_rate."',";
			}
			if($agent_rate){
				$condition .= " agent_rate = '".$agent_rate."',";
			}
			...
			if($res){
				...
				bmsAlert("操作成功",pfUrl("bms","deal","parameter")); 
			}else{
				bmsAlert("操作失败",pfUrl("bms","deal","parameter")); 
			}
		}
		$detail = $Model->query("select * from `jjs_deal_setting`");
	...
	public function index()
	{
		...
		$urlrule = 'index.php?c=deal&a=index';
		if($cate == "买入"){
			$where .= " and C.cate = 0";
			$urlrule = 'index.php?c=deal&a=index&cate=买入';
		}elseif($cate == "卖出"){
			$where .= " and C.cate = 1";
			$urlrule = 'index.php?c=deal&a=index&cate=卖出';
		}

		if($currenttime){
			$where .= " and C.deal_time like '%".$currenttime."%'";
			$urlrule = 'index.php?c=deal&a=index&currenttime='.$currenttime;
		}

		if($starttime && $overtime){
			$where .= " and C.deal_time between '".$starttime."' and '".$overtime."' ";
			$urlrule = 'index.php?c=deal&a=index&starttime='.$starttime.'&overtime='.$overtime;
		}elseif($starttime && empty($overtime)){
			$where .= " and C.deal_time >= '".$starttime."'";
			$urlrule = 'index.php?c=deal&a=index&starttime='.$starttime;
		}elseif(empty($starttime) && $overtime){
			$where .= " and C.deal_time <= '".$overtime."'";
			$urlrule = 'index.php?c=deal&a=index&overtime='.$overtime;
		}
